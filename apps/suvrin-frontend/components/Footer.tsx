"use client";

import { useState } from "react";
import Link from "next/link";
import Image from "next/image";

export function Footer() {
  const [openSections, setOpenSections] = useState<{ [key: string]: boolean }>({
    about: false,
    services: false,
    caseStudies: false,
  });

  const toggleSection = (section: string) => {
    setOpenSections((prev) => {
      const isCurrentlyOpen = prev[section];
      return {
        about: section === "about" ? !isCurrentlyOpen : false,
        services: section === "services" ? !isCurrentlyOpen : false,
        caseStudies: section === "caseStudies" ? !isCurrentlyOpen : false,
      };
    });
  };

  const offices = [
    {
      city: "Dubai (UAE)",
      name: "MAXCODE SOLUTIONS LLC",
      address: "Living, Marina Gate, D94 M Floor, Jumeirah, Dubai Marina",
      phone: "+971 551120975",
    },
    {
      city: "INDIA",
      name: "Gurgaon",
      address: "P1/55, DLF Phase 2, Cyber City, Gurgaon",
      phone: "+91 9811021616",
    },
    {
      city: "USA",
      name: "Hoboken, NJ",
      address: "Riverfront Center, 221 River St 9th Floor, Hoboken, NJ 07030",
      phone: "+1 917 689 3693",
    },
  ];

  const news = [
    {
      title:
        "Choosing the Right ERP Solution for Businesses in India and the Middle East",
      image:
        "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=2426&ixlib=rb-4.0.3",
      link: "#",
    },
    {
      title:
        "Odoo's Meteoric Rise in India: From Open-Source Roots to Enterprise Dominance",
      image:
        "https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=2340&ixlib=rb-4.0.3",
      link: "#",
    },
  ];

  return (
    <footer
      className="bg-white text-gray-900 pt-6 pb-4 md:pt-10 md:pb-6 font-sans relative"
      id="site-footer"
    >
      <div className="max-w-[1240px] mx-auto px-6">
        {/* Main Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-4 border-b border-gray-200">
          {/* Column 1: Brandt & Partners */}
          <div className="flex flex-col gap-2 items-center md:items-start">
            <Link href="/" className="inline-block md:self-start">
              <Image
                src="/suvrin-logo.png"
                alt="Suvrin Technologies Logo"
                width={200}
                height={70}
                className="h-16 md:h-20 w-auto object-contain"
                priority
              />
            </Link>

            <div className="flex flex-col gap-12 pt-4">
              <div className="relative w-full max-w-[140px]">
                <Image
                  src="/clutch.svg"
                  alt="Partner Logos"
                  width={200}
                  height={300}
                  className="w-full h-auto object-contain"
                />
              </div>
            </div>
          </div>

          {/* Column 2: Addresses */}
          <div className="flex flex-col gap-4 items-center md:items-start text-center md:text-left">
            {offices.map((office, idx) => (
              <div key={idx} className="flex flex-col gap-1">
                <h4 className="text-[0.75rem] font-bold text-gray-500 uppercase tracking-widest">
                  {office.city}
                </h4>
                <p className="text-[0.85rem] font-bold text-gray-900">
                  {office.name}
                </p>
                <p className="text-[0.8rem] text-gray-600 leading-relaxed max-w-[200px]">
                  {office.address}
                </p>
                <p className="text-[0.8rem] font-bold text-gray-900 mt-0.5">
                  {office.phone}
                </p>
              </div>
            ))}
          </div>

          {/* Column 3: Navigation */}
          <div className="flex flex-col gap-3 items-center md:items-start w-full text-center md:text-left">
            <div className="flex flex-col gap-3 w-full">
              <h3 className="text-[1rem] font-bold border-b border-gray-200 pb-1 text-gray-900">
                Navigation
              </h3>
              <ul className="flex flex-col gap-2">
                {/* Home */}
                <li>
                  <Link
                    href="/"
                    className="text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900"
                  >
                    Home
                  </Link>
                </li>

                {/* About Section - FIXED ALIGNMENT */}
                <li className="flex flex-col items-center md:items-start gap-1.5">
                  <button
                    onClick={() => toggleSection("about")}
                    className="flex items-center justify-center md:justify-start gap-1 w-auto text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900 cursor-pointer"
                  >
                    <span>About</span>
                    <span
                      className={`text-[0.6rem] transition-transform duration-300 ml-1 ${openSections.about ? "rotate-180" : ""}`}
                    >
                      ▼
                    </span>
                  </button>
                  <div
                    className={`overflow-hidden transition-all duration-300 ${openSections.about ? "max-h-[200px] opacity-100 mt-2" : "max-h-0 opacity-0"}`}
                  >
                    <ul className="flex flex-col gap-1 pl-0 md:pl-3 border-l-0 md:border-l border-gray-200 items-center md:items-start">
                      <li>
                        <Link
                          href="/about"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Who We Are
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/about/team"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Team
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/about/careers"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Careers
                        </Link>
                      </li>
                    </ul>
                  </div>
                </li>

                {/* Services Section - FIXED ALIGNMENT */}
                <li className="flex flex-col items-center md:items-start gap-1.5">
                  <button
                    onClick={() => toggleSection("services")}
                    className="flex items-center justify-center md:justify-start gap-1 w-auto text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900 cursor-pointer"
                  >
                    <span>Services</span>
                    <span
                      className={`text-[0.6rem] transition-transform duration-300 ml-1 ${openSections.services ? "rotate-180" : ""}`}
                    >
                      ▼
                    </span>
                  </button>
                  <div
                    className={`overflow-hidden transition-all duration-300 ${openSections.services ? "max-h-[350px] opacity-100 mt-2" : "max-h-0 opacity-0"}`}
                  >
                    <ul className="flex flex-col gap-1.5 pl-0 md:pl-3 border-l-0 md:border-l border-gray-200 items-center md:items-start">
                      <li>
                        <Link
                          href="/service-scope/application-development"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Application Development
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/cybersecurity"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Cybersecurity
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/digital-transformation"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Digital Transformation
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/software-engineering"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Software Engineering
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/enterprise-applications"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Enterprise Applications
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/staff-augmentation"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Staff Augmentation
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/support-services"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Support Services
                        </Link>
                      </li>
                    </ul>
                  </div>
                </li>

                {/* Case Studies Section - FIXED ALIGNMENT */}
                <li className="flex flex-col items-center md:items-start gap-1.5">
                  <button
                    onClick={() => toggleSection("caseStudies")}
                    className="flex items-center justify-center md:justify-start gap-1 w-auto text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900 cursor-pointer"
                  >
                    <span>Case Studies</span>
                    <span
                      className={`text-[0.6rem] transition-transform duration-300 ml-1 ${openSections.caseStudies ? "rotate-180" : ""}`}
                    >
                      ▼
                    </span>
                  </button>
                  <div
                    className={`overflow-hidden transition-all duration-300 ${openSections.caseStudies ? "max-h-[450px] opacity-100 mt-2" : "max-h-0 opacity-0"}`}
                  >
                    <ul className="flex flex-col gap-1.5 pl-0 md:pl-3 border-l-0 md:border-l border-gray-200 items-center md:items-start">
                      <li>
                        <Link
                          href="/service-scope/strategy-advisory"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Strategy & Advisory
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/ai-products-platforms"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          AI Products & Platforms
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/commercialization-growth"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Commercialization & Growth
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/agents"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Agents
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/knowledge-systems"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Knowledge Systems
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/automation-integration"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Automation & Integration
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/ai-infrastructure"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          AI Infrastructure
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/governance-risk"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Governance & Risk
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/llm-fine-tuning"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          LLM Fine-Tuning
                        </Link>
                      </li>
                      <li>
                        <Link
                          href="/service-scope/web-scraping"
                          className="text-[0.8rem] text-gray-600 hover:text-gray-900 transition-colors"
                        >
                          Web-Scraping
                        </Link>
                      </li>
                    </ul>
                  </div>
                </li>

                {/* Blog & Contact */}
                <li>
                  <Link
                    href="/blog"
                    className="text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900"
                  >
                    Blog
                  </Link>
                </li>
                <li>
                  <Link
                    href="/contact"
                    className="text-[0.9rem] font-bold hover:text-gray-600 transition-colors text-gray-900"
                  >
                    Contact
                  </Link>
                </li>
              </ul>
            </div>
          </div>

          {/* Column 4: Latest News */}
          <div className="flex flex-col gap-4 items-center md:items-start w-full text-center md:text-left">
            <h3 className="text-[1rem] font-bold text-gray-900">Latest news</h3>
            <div className="flex flex-col gap-6 w-full items-center md:items-start">
              {news.map((item, idx) => (
                <Link
                  key={idx}
                  href={item.link}
                  className="flex flex-col items-center md:items-start gap-4 group"
                >
                  <div className="relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border border-gray-300">
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                      sizes="(max-width: 768px) 80px, 80px"
                      className="object-cover transition-transform duration-500 group-hover:scale-110"
                    />
                  </div>
                  <h4 className="text-[0.8rem] font-medium leading-[1.4] hover:text-gray-600 transition-colors text-gray-900">
                    {item.title}
                  </h4>
                </Link>
              ))}
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="flex flex-col md:flex-row justify-between items-center mt-6 gap-4 text-center md:text-left">
          <div className="flex items-center gap-4">
            <Link href="#" aria-label="Facebook">
              <Image
                src="/Twitter logo.avif"
                alt="Facebook"
                width={24}
                height={24}
                className="object-contain hover:opacity-70 transition-opacity"
              />
            </Link>
            <Link href="#" aria-label="Twitter">
              <Image
                src="/facebook Logo.avif"
                alt="Twitter"
                width={24}
                height={32}
                className="object-contain hover:opacity-70 transition-opacity"
              />
            </Link>
            <Link href="#" aria-label="Instagram">
              <Image
                src="/instagram logo.avif"
                alt="Instagram"
                width={28}
                height={28}
                className="object-contain hover:opacity-70 transition-opacity"
              />
            </Link>
          </div>

          <div className="text-[0.8rem] font-medium text-gray-600">
            In Collaboration with{" "}
            <Link
              href="#"
              className="text-gray-900 hover:underline underline-offset-4 font-bold transition-all"
            >
              Maxcode Solutions
            </Link>
          </div>

          <div className="text-[0.75rem] text-gray-500 order-3 md:order-none">
            © 2026 Suvrin Technologies.
          </div>
        </div>
      </div>

      {/* WhatsApp FAB */}
      <Link
        href="https://wa.me/yournumber"
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-8 right-8 w-16 h-16 bg-[#25D366] rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform z-[1001]"
        aria-label="Chat on WhatsApp"
      >
        <svg
          width="36"
          height="36"
          viewBox="0 0 24 24"
          fill="white"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      </Link>
    </footer>
  );
}
